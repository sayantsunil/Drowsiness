...
⚙️ How It Works
The webcam captures frames of the driver's face.
MediaPipe detects 3D facial landmarks, especially the eyes.
The Eye Aspect Ratio (EAR) is calculated:
Low EAR = Eyes are closed
If low EAR persists > 4 seconds → Drowsiness detected
An alarm is triggered using a sound to alert the driver.
Eye Aspect Ratio (EAR)
The EAR is calculated using the distance between six specific points around each eye: A = ||p2 - p6|| B = ||p3 - p5|| C = ||p1 - p4||
